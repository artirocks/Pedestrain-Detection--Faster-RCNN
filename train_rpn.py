...
parser.add_option("--network", dest="network", help="Base network to use. Supports vgg or resnet50.", default='mobilenetv1')
...
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='/storage/research/Intern19_v2/Team29/frcnn_weights/rpn/rpn_frcnn.hdf5')
parser.add_option("--input_weight_path", dest="input_weight_path", help="Input path for weights. If not specified, will try to load default weights provided by keras.",default='mobilenet_1_0_224_tf.h5')
...
# compile model
optimizer = Adam(lr=1e-5, decay=1e-8)
...
Callbacks=keras.callbacks.ModelCheckpoint(""+options.network+".weights.{epoch:02d}-{val_loss:.2f}.hdf5", monitor='val_loss', verbose=1, save_best_only=False, save_weights_only=True, mode='auto', period=50)
callback=[Callbacks]
history = model_rpn.fit_generator(data_gen_train,
                    epochs=options.num_epochs, validation_data=data_gen_val,
                    steps_per_epoch=250,callbacks=callback, validation_steps=10)

model_json = model_rpn.to_json()
...
numpy.savetxt(""+"_rpn_loss_history.txt", numpy_loss_history, delimiter=",")
